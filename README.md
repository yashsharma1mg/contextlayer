...
**A local-first workspace that turns scattered product knowledge into cited decisions and compilable React — on one Mac, with the evidence attached.**

[![CI](https://github.com/yashsharma1mg/contextlayer/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/yashsharma1mg/contextlayer/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
![Platform](https://img.shields.io/badge/platform-macOS%2013%2B%20·%20Apple%20Silicon-lightgrey)

[Verification](#verification) · [What it does](#what-it-does) · [Architecture](#architecture) · [Install](#macos-application) · [Privacy](#privacy-and-consent) · [Development](#development) · [Boundaries](#current-boundaries)

</div>

---

Documents, product captures, design-system assets, flows, requirements, reviews, and generated interfaces live together as a connected project graph. Every answer cites its sources. Every generated interface is validated against a pinned design-system version before it can compile, preview, or publish.
...
<!-- Screenshots go here: canvas hero, redaction preview, UI plan -> generated React. -->
...
A notch-anchored HUD window can capture the screen, read the accessibility element under the cursor, fall back to OCR for text the accessibility tree does not expose, and draw a click-through cursor companion. It is implemented across [`screen.rs`](apps/desktop/src-tauri/src/screen.rs), [`pointer.rs`](apps/desktop/src-tauri/src/pointer.rs), and a Swift `screen-agent` helper, exposed as four Tauri commands — `screen_permissions`, `screen_capture`, `screen_element`, `screen_copy_mode` — under explicit capability manifests.

Nothing is captured until the HUD is toggled on from the canvas, and macOS independently gates it behind Screen Recording and Accessibility permission, which the user grants in System Settings.
...
Context Layer stores its runtime data under `~/Library/Application Support/Context Layer`:
...
The desktop runtime creates a timestamped PostgreSQL dump before applying migrations to an existing database. To restore one:
...
The runtime accepts restore files **only from its own backup directory**, restores before migrations, and removes the request marker after success. Keep a separate copy of the entire data directory for device-level backup.
...
The release gate on an 8 GB Apple Silicon Mac:

| Metric | Ceiling |
| --- | --- |
| Settled idle CPU | under **2.8%** |
| Total idle memory | under **1050 MB** |
| Surviving child processes after quit | **none** |

Enforced by [`resource-gate.ts`](apps/desktop/scripts/resource-gate.ts), which launches the built app, waits for health, samples settled usage, quits, and fails on survivors:

```bash
bun run --cwd apps/desktop test:resource
```

The gate runs at release rather than in CI, because it requires a built macOS bundle. The ceiling was raised from 2% / 750 MB to make room for the screen-awareness surface above; if that surface does not survive, the ceiling returns.
...
`GitHub repositories` · `Notion workspaces` · `Google Drive` · `Slack` · `Confluence` · `Figma files and read-only libraries` · `Allowlisted Streamable HTTP MCP servers` · `Web URLs`
...
The manifest can include package imports, browser and CSS entries, foundations, tokens, primitives, components, patterns, templates, props, variants, slots, examples, accessibility guidance, composition constraints, and source mappings. Importers support package metadata and browser-compatible archives, Storybook, read-only Figma libraries and variables, and Code Connect mappings.

Package inspection blocks install scripts, path traversal, executable macros, unbounded archive expansion, arbitrary network access, and known unbounded execution patterns.
...
[`apps/context-agent/.env.example`](apps/context-agent/.env.example) documents database, Better Auth, encryption, model, connector, and OAuth values. At minimum, development requires `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `STUDIO_URL`, and `CONNECTION_ENCRYPTION_KEY`.
...
> [!NOTE]
> Stated plainly, because a boundary you can read is worth more than one you discover.

- The first release is **single-device and local-first**. Cloud sync, networked multiplayer, and live cursors are deferred.
- **Screen awareness is experimental and untested.** See [the section above](#screen-awareness).
- **Team scoping was removed.** Visibility is personal or organization; teams were half-wired and unreachable from the UI, so they went rather than being finished.
